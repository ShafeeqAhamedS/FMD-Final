...
const App = () => {
  ...
  const checkHealth = async () => {
    try {
      ...
      setError("Health check failed. The backend may be unavailable.") // Display health check error
    }
  }
  ...
  const fetchMetrics = async () => {
    try {
      ...
      setMetrics(null)
      console.error("Failed to fetch metrics:", err); // Log the error
    }
  }
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // Input Validation: Ensure 'Hours' is a valid number
      if (isNaN(convertedInputs.Hours)) {
      ...
      let errorMessage = "Prediction failed. Please check your input and try again."; // Default error message

      if (err.message) {
        errorMessage = err.message; // Use error message from input validation
      } else if (axios.isAxiosError(err)) {
        errorMessage = err.response?.data?.detail || "Prediction failed due to a backend error."; // Check for backend specific error
      }
      ...
      <div className="max-w-xs sm:max-w-md md:max-w-lg lg:max-w-xl xl:max-w-2xl mx-auto bg-white rounded-xl shadow-md overflow-hidden">
        <div className="p-6 sm:p-8 md:p-10">
          ...
          <h1 className="text-lg sm:text-xl md:text-2xl lg:text-3xl font-medium text-black mb-2">
            Student Score Prediction {/* Update the title */}
          </h1>
          <p className="mt-2 text-sm sm:text-base text-gray-500">
            Predict student scores based on the number of study hours. {/* Update description */}
          </p>
          ...
          {prediction && (
            <div className="mt-6">
              <p className="text-xl sm:text-2xl font-semibold text-gray-800">
                Predicted Score: {prediction[0].toFixed(2)}
              </p>
            </div>
          )}
